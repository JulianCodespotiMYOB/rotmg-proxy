...
const packet_map_1 = require("../stuff not being used/packet-map");
const IMPORTANT_PACKET_IDS = new Set([30]);
function logPacket(data, isClientToServer, packetStats, packetSequence) {
    try {
        // Only log properly formatted packets
        if (data.length >= 5) {
            const packetSize = data.readInt32BE(0);
            const packetId = data[4];
            const packetType = packet_map_1.PACKET_MAP[packetId] || 'Unknown';
        ...
        `);
                ...
                if (packetId === 0) { // FAILURE
                    ...
                    console.log("Packet sequence leading to failure:", JSON.stringify(packetSequence.slice(-20), null, 2)); // Show last 20 packets
                }
