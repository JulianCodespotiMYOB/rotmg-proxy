...
class RotmgHttpTunnelProxy {
  ...
  private handleConnection(clientSocket: net.Socket): void {
    ...
    let targetPort: number = 443;
    let serverSocket: net.Socket | null = null;
    let connectEstablished = false;
    let useEncryption = false;

    // Create RC4 ciphers
    const clientToServerCipher = new CustomRC4(CLIENT_SEND_KEY);
    const serverToClientCipher = new CustomRC4(SERVER_SEND_KEY);
     ...
     */
    function setupServerSocketHandlers(socket: net.Socket): void {
      socket.on('data', (data) => {
        try {
     ...
     */
    function isGamePacket(data: Buffer): boolean {
     ...
     */
    function processGamePacket(data: Buffer, isClientToServer: boolean): void {
      try {
